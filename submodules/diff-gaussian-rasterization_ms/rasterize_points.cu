  ...
  )
{
  ...
  if(P != 0)
  {
    ...
    rendered = CudaRasterizer::Rasterizer::forward(
      ...
      theta.contiguous().data<float>(),
      phi.contiguous().data<float>(),
      w_fg.contiguous().data<float>(),
  ...
  const bool debug) 
{
  ...
  torch::Tensor dL_dtheta = torch::zeros({P, 1}, means3D.options());
  torch::Tensor dL_dphi = torch::zeros({P, 1}, means3D.options());
  ...
  {  
    ...
    dL_dopacity.contiguous().data<float>(),
      dL_dtheta.contiguous().data<float>(),
	  dL_dphi.contiguous().data<float>(),
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacity,  dL_dtheta, dL_dphi, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations);
}
